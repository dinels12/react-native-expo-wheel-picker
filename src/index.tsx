...
import type { ItemType, IViuPickerProps, IViuPickerState, RenderItemProps } from "./types";
...
class WheelPickerExpo extends PureComponent<IViuPickerProps, IViuPickerState> {
  ...
  get gradientContainerStyle() {
    ...
    return [{ height: 2 * itemHeight, borderColor: selectedStyle?.borderColor }, styles.gradientContainer];
  }
  ...
  setData() {
    ...
    if (items?.length) {
      ...
      const data = [additionalItem, additionalItem, ...items, additionalItem, additionalItem] as ItemType[];
  ...
  render() {
    ...
    const { width, initialSelectedIndex, flatListProps, selectedStyle } = this.props;
    ...
    return (
      ...
      >
        ...
        />
        <View
          style={[this.gradientContainerStyle, styles.topGradient, { borderBottomWidth: selectedStyle?.borderWidth }]}
          pointerEvents="none"
        >
          <LinearGradient style={styles.linearGradient} colors={[this.backgroundColor, this.gradientColor]} />
        </View>
        <View
          style={[this.gradientContainerStyle, styles.bottomGradient, { borderTopWidth: selectedStyle?.borderWidth }]}
          pointerEvents="none"
        >
          <LinearGradient style={styles.linearGradient} colors={[this.gradientColor, this.backgroundColor]} />
        </View>
      </View>
    );
  }
...
const Item = React.memo(({ fontSize, label, fontColor, textAlign }: RenderItemProps) => (
...
const PickerItem = (
  ...
  renderItem?: (props: RenderItemProps) => any
) => {
    ...
    <TouchableOpacity activeOpacity={1} onPress={() => onPress(index - 2)}>
      <View style={style}>
        {renderItem ? (
          ...
          <Item fontSize={fontSize} fontColor={fontColor} textAlign={textAlign} label={item.label} />
        )}
      </View>
    </TouchableOpacity>
  );
